...
function createModel(userCount, articleCount, categoryCount, embeddingSize) {
    ...
    const categoryEmbedding = tf.layers.dense({units: embeddingSize, activation: 'relu'}).apply(categoryInput);

    const concatenated = tf.layers.concatenate().apply([tf.layers.flatten().apply(userEmbedding), tf.layers.flatten().apply(articleEmbedding), tf.layers.flatten().apply(categoryEmbedding)]);
